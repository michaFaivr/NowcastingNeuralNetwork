...
%///////////////////////
%5. a priori processing
%///////////////////////
%%% if RAD1 is between Rad1(classk)_mean +/-2.5sigmas -> Wk,l=100
%%% else : Wk,l = 0
RAD_mean=NaN(NBcla,1);
RAD_sigma=NaN(NBcla,1);
WEIGHTS_IwpRad1=100*ones(size(IWP,1),size(IWP,2)); %%%dim ok
whos WEIGHT_IwpRad
alpha=1.;%2.5
%pause

%calculate RADk_mean and RADk_sigma for each IWPclass
%%% loop on NBcla
for NN = 1:NBcla-1
%%%
    thisIWPclSet = find(IWP>=threshvect(NN) & IWP<threshvect(NN+1));
    RAD1_mean(NN,1)=mean(RAD1(thisIWPclSet));
    RAD1_sigma(NN,1)=std(RAD1(thisIWPclSet));

    Outliers=find(RAD1(thisIWPclSet) < RAD1_mean(NN,1)-alpha*RAD1_sigma(NN,1) | ...
    ...
    numel(Outliers)
%    pause
    WEIGHTS_IwpRad1(thisIWPclSet(Outliers),1)=0;
    clear thisIWPclSet Outliers
%%%
end  %for NN 
%%%
disp('WEIGHTS_IwpRad after NN loop')
whos WEIGHTS_IwpRad1   %%%dim error
%pause
